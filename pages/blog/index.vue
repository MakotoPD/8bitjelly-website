				...
				<NuxtLink :to="localePath('/blog/'+wpis.attributes.slug)" v-for="wpis in blog.data" 
					...
					<div class="flex px-4 pt-2">
						<div v-for="tag in  wpis.attributes.tags.data">
							<p class="px-2 py-0.5 bg-darker/30 rounded-full text-sm text-darker flex items-center gap-0.5">
...
const fetchBlog = async () => {
    const { data, pending } = await useFetch(
        `https://panel.8bitjelly.com/api/blogs?sort[0]=createdAt:desc&locale[0]=${lang.value}&populate[tags][fields][0]=name&populate[zdjecie]=true&fields[0]=Tytul&fields[1]=autor&fields[2]=locale&fields[3]=publishedAt&fields[4]=blog&fields[5]=slug`
    )
...
const fetchTags = async () => {
    const { data, pending } = await useFetch(
        `https://panel.8bitjelly.com/api/blog-tags?locale[0]=${lang.value}&fields[0]=name`
    )
