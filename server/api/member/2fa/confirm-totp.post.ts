...
export default defineEventHandler(async (event) => {
  ...
  if (!acc?.totpSecret) throw createError({ statusCode: 400, message: 'Najpierw uruchom setup TOTP' })

  if (!verifyTotp(acc.totpSecret, code)) throw createError({ statusCode: 401, message: 'Nieprawidłowy kod' })

  await db.update(accountUsers).set({ totpEnabled: true }).where(eq(accountUsers.id, account.accountId))
  return { ok: true }
})
